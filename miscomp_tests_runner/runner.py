...
regular_crashed_log = f"{log_path}/regular_crashed.txt"

crc_executed_log = f"{log_path}/crc_executed.txt"
crc_crashed_log = f"{log_path}/crc_crashed.txt"
crc_inject_failed_log = f"{log_path}/crc_inject_failed.txt"
...
open(regular_crashed_log, "w").close()
open(crc_executed_log, "w").close()
open(crc_crashed_log, "w").close()
open(crc_inject_failed_log, "w").close()
...
def run_main(id, f1, f2, opt):
    ...
    try:
        res = subprocess.run(
            cmd, capture_output=True, text=True, timeout=timeout_duration
        )
    ...
    if "REGULAR CRASHED" in res.stdout:
        stats["regular_crashed"] += 1
        add_id_to_log(regular_crashed_log, f"{id}_{opt}")
    ...
    if "CRC FAILED. unable to inject crc" in res.stdout:
        stats["crc_inject_failed"] += 1
        add_id_to_log(crc_inject_failed_log, f"{id}_{opt}")
...
def run_script_on_files(directory_path):
    ...
    for filename in os.listdir(directory_path):
        ...
        if match:
            ...
            file_map[file_id][opt_level][opt_type] = os.path.join(
    ...
    for file_id in sorted_file_ids:
        ...
        if (
            "O0" in opt_levels
            and "base" in opt_levels["O0"]
            and "opt" in opt_levels["O0"]
        ):
        ...
        if (
            "O1" in opt_levels
            and "base" in opt_levels["O1"]
            and "opt" in opt_levels["O1"]
        ):
