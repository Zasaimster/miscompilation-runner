...
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/174845977588921701794216300268120761476_O1.ll'
...
if.end9:                                          ; preds = %if.then8, %entry
  ...
  %dy.1 = phi i64 [ 0, %if.then8 ], [ %dy.0, %entry ]
  %dx.1 = phi i64 [ %dy.0, %if.then8 ], [ 0, %entry ]
...
if.then19:                                        ; preds = %if.end9
  ...
  %spec.select = select i1 %tobool22.not, i32 %cond21, i32 %xor
  br label %if.end38
...
if.else:                                          ; preds = %if.end9
  ...
  %or.cond61 = select i1 %cmp25.not, i1 true, i1 %cmp28.not
  br i1 %or.cond61, label %if.end38, label %if.then29

if.then29:                                        ; preds = %if.else
  %cond31 = select i1 %cmp, i32 8, i32 4
  %tobool32.not = icmp eq i32 %yi.0, 0
  %xor34 = xor i32 %cond31, 12
  %spec.select62 = select i1 %tobool32.not, i32 %cond31, i32 %xor34
  br label %if.end38

if.end38:                                         ; preds = %if.then29, %if.else, %if.then19
  %hints.0 = phi i32 [ %spec.select, %if.then19 ], [ %spec.select62, %if.then29 ], [ 0, %if.else ]
  ret i32 %hints.0
}
