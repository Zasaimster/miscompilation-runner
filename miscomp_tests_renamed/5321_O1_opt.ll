...
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/111551859500312642059360352826008361492_O1.ll'
...
for.cond:                                         ; preds = %for.inc, %entry
  %cnt0.0 = phi i32 [ 8, %entry ], [ %cnt0.1, %for.inc ]
...
if.else:                                          ; preds = %for.cond
  %inc4 = add nsw i32 %cnt1.0, 1
  br label %for.inc

for.inc:                                          ; preds = %if.else, %if.then
  %cnt0.1 = phi i32 [ %inc, %if.then ], [ %cnt0.0, %if.else ]
  %cnt1.1 = phi i32 [ %cnt1.0, %if.then ], [ %inc4, %if.else ]
  %dec = add nsw i32 %i.0, -1
  br label %for.cond, !llvm.loop !5

for.end:                                          ; preds = %for.cond
  %cmp5 = icmp ne i32 %cnt0.0, 1
  %cmp6 = icmp ne i32 %cnt1.0, 7
