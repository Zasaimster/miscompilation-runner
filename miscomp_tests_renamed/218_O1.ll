; 103738805691550497794632394945491015061
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/103738805691550497794632394945491015061.c'
source_filename = "/mnt/ramtmp/optims/DCE.cpp/target/103738805691550497794632394945491015061.c"
...
entry:
  ...
  %cmp20 = icmp slt i32 %c.promoted, 2
...
for.body:                                         ; preds = %for.body.lr.ph, %for.inc
  %timeout.023 = phi i32 [ 0, %for.body.lr.ph ], [ %timeout.2, %for.inc ]
  %e.022 = phi i32 [ 2, %for.body.lr.ph ], [ %e.3, %for.inc ]
  %inc121921 = phi i32 [ %c.promoted, %for.body.lr.ph ], [ %inc12, %for.inc ]
  %tobool.not = icmp ugt i32 %e.022, -8
...
if.end:                                           ; preds = %if.then, %for.body
  %e.1 = phi i32 [ %not3, %if.then ], [ %e.022, %for.body ]
  %umax = tail call i32 @llvm.umax.i32(i32 %e.1, i32 94)
  br label %while.cond

while.cond:                                       ; preds = %while.body, %if.end
  %e.2 = phi i32 [ %e.1, %if.end ], [ %inc, %while.body ]
  %timeout.1 = phi i32 [ %timeout.023, %if.end ], [ %inc7, %while.body ]
...
for.inc:                                          ; preds = %cleanup
  %inc12 = add i32 %inc121921, 1
  store i32 %inc12, ptr @c, align 4, !tbaa !5
  %exitcond24.not = icmp eq i32 %inc12, 2
  br i1 %exitcond24.not, label %for.end, label %for.body, !llvm.loop !13

for.end:                                          ; preds = %for.inc, %entry
  %.lcssa = phi i32 [ %c.promoted, %entry ], [ 2, %for.inc ]
  %call = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, i32 noundef %.lcssa)
  br label %cleanup13
...
cleanup13:                                        ; preds = %cleanup, %for.end
  ret i32 0
}
