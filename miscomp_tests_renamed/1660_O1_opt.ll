...
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/140225981822445092929223414178750002085_O1.ll'
...
if.else:                                          ; preds = %entry
  %cmp12.not = icmp eq i64 %sub11, 0
  %. = select i1 %cmp12.not, i32 2, i32 1
  br label %cleanup

cleanup:                                          ; preds = %if.else, %if.then
  %retval.0 = phi i32 [ undef, %if.then ], [ %., %if.else ]
  ret i32 %retval.0
}
