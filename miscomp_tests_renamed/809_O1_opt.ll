; 114247088075506582824266888806885450961
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/114247088075506582824266888806885450961_O1.ll'
source_filename = "/mnt/ramtmp/optims/DCE.cpp/target/114247088075506582824266888806885450961.c"
...
; Function Attrs: nofree nounwind uwtable
define dso_local noundef i32 @foo(ptr noundef readnone captures(none) %a, i64 noundef %b, i32 noundef %c) local_unnamed_addr #1 {
entry:
  %cmp = icmp sgt i64 %b, 20
...
if.then.backedge:                                 ; preds = %land.lhs.true25, %land.lhs.true20, %if.then
  br label %if.then

land.lhs.true25:                                  ; preds = %if.end17
  br i1 %brmerge84, label %if.then.backedge, label %for.body

for.body:                                         ; preds = %land.lhs.true25, %land.lhs.true20
  tail call void @abort() #6
  unreachable
