; 163983654217894593163976693329473197790
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/163983654217894593163976693329473197790_O1.ll'
source_filename = "/mnt/ramtmp/optims/DCE.cpp/target/163983654217894593163976693329473197790.c"
...
define dso_local ptr @server_type(ptr noundef readnone captures(none) %a, ptr noundef readnone captures(none) %b, ptr noundef readonly captures(none) %arg) local_unnamed_addr #1 {
entry:
  %call = tail call i32 (i32, ...) @example6(i32 noundef 0) #4
...
if.end:                                           ; preds = %entry
  %call1 = tail call i32 @strcmp(ptr noundef nonnull dereferenceable(1) %arg, ptr noundef nonnull dereferenceable(6) @.str) #4
  %tobool2.not = icmp eq i32 %call1, 0
  br i1 %tobool2.not, label %if.end9, label %if.else

if.else:                                          ; preds = %if.end
  %call4 = tail call i32 @strcmp(ptr noundef nonnull dereferenceable(1) %arg, ptr noundef nonnull dereferenceable(11) @.str.1) #4
...
cleanup:                                          ; preds = %if.end9, %if.else, %if.then
  %retval.0 = phi ptr [ %0, %if.then ], [ null, %if.end9 ], [ @.str.2, %if.else ]
  ret ptr %retval.0
}

declare i32 @example6(...) local_unnamed_addr #2
...
entry:
  %call.i = tail call i32 (i32, ...) @example6(i32 noundef 0) #4
