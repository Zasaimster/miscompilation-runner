; 121816600977410321185623292961464366119
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/121816600977410321185623292961464366119.c'
source_filename = "/mnt/ramtmp/optims/DCE.cpp/target/121816600977410321185623292961464366119.c"
...
entry:
  %call12 = tail call i32 (...) @example7() #8
  %cmp13 = icmp eq i32 %call12, 0
  br i1 %cmp13, label %return, label %if.end

if.end:                                           ; preds = %entry, %if.end5
  %y.addr.015 = phi i64 [ %rem6, %if.end5 ], [ %y, %entry ]
  %x.addr.014 = phi i64 [ %rem, %if.end5 ], [ %x, %entry ]
  %call1 = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, i64 noundef %x.addr.014)
  %conv = sext i32 %call1 to i64
  %rem = urem i64 %conv, %y.addr.015
  %cmp2 = icmp eq i64 %rem, 0
  br i1 %cmp2, label %return, label %if.end5

if.end5:                                          ; preds = %if.end
  %rem6 = urem i64 %y.addr.015, %rem
  %call = tail call i32 (...) @example7() #8
  %cmp = icmp eq i32 %call, 0
  br i1 %cmp, label %return, label %if.end, !llvm.loop !5

return:                                           ; preds = %if.end5, %if.end, %entry
  %retval.0 = phi i64 [ %x, %entry ], [ %rem, %if.end5 ], [ %y.addr.015, %if.end ]
  ret i64 %retval.0
}
...
define dso_local i64 @powmod_ll(i64 noundef %b, i32 noundef %e, i64 noundef %m) local_unnamed_addr #3 {
entry:
...
; Function Attrs: nounwind uwtable
define dso_local void @facts(i64 noundef %t, i32 noundef %a_int, i32 noundef %x0, i32 noundef %p) local_unnamed_addr #0 {
entry:
  ...
  %switch116 = icmp ult i32 %p, 2
  br label %for.body

for.body:                                         ; preds = %entry, %for.inc
  %jj.0110 = phi i32 [ 0, %entry ], [ %jj.1, %for.inc ]
  %j.0109 = phi i32 [ 1, %entry ], [ %j.1, %for.inc ]
  %t.addr.0108 = phi i64 [ %t, %entry ], [ %t.addr.1, %for.inc ]
  %i.0107 = phi i32 [ 1, %entry ], [ %inc, %for.inc ]
  %xp.0106 = phi ptr [ @factab, %entry ], [ %xp.1, %for.inc ]
  %q.0105 = phi i64 [ 1, %entry ], [ %rem, %for.inc ]
  %y.0104 = phi i64 [ %conv1, %entry ], [ %add7, %for.inc ]
  %x.0103 = phi i64 [ %conv1, %entry ], [ %add, %for.inc ]
  %.mux = select i1 %cmp.i, i64 1, i64 %x.0103
  br i1 %switch, label %powmod_ll.exit, label %for.body4.i

for.body4.i:                                      ; preds = %for.body, %for.inc9.i
  %i.129.in.i = phi i32 [ %i.129.i, %for.inc9.i ], [ %2, %for.body ]
  %pow.028.i = phi i64 [ %pow.1.i, %for.inc9.i ], [ %x.0103, %for.body ]
  %i.129.i = add nsw i32 %i.129.in.i, -1
  %mul.i = mul i64 %pow.028.i, %pow.028.i
  %rem.i = urem i64 %mul.i, %t.addr.0108
...
if.then5.i:                                       ; preds = %for.body4.i
  %mul6.i = mul i64 %rem.i, %x.0103
  %rem7.i = urem i64 %mul6.i, %t.addr.0108
  br label %for.inc9.i
...
powmod_ll.exit:                                   ; preds = %for.inc9.i, %for.body
  ...
  %.mux112 = select i1 %cmp.i, i64 1, i64 %y.0104
  br i1 %switch115, label %powmod_ll.exit79, label %for.body4.i63

for.body4.i63:                                    ; preds = %powmod_ll.exit, %for.inc9.i75
  %i.129.in.i64 = phi i32 [ %i.129.i66, %for.inc9.i75 ], [ %5, %powmod_ll.exit ]
  %pow.028.i65 = phi i64 [ %pow.1.i76, %for.inc9.i75 ], [ %y.0104, %powmod_ll.exit ]
  %i.129.i66 = add nsw i32 %i.129.in.i64, -1
  %mul.i67 = mul i64 %pow.028.i65, %pow.028.i65
  %rem.i68 = urem i64 %mul.i67, %t.addr.0108
...
if.then5.i72:                                     ; preds = %for.body4.i63
  %mul6.i73 = mul i64 %rem.i68, %y.0104
  %rem7.i74 = urem i64 %mul6.i73, %t.addr.0108
...
powmod_ll.exit79:                                 ; preds = %for.inc9.i75, %powmod_ll.exit
  %retval.0.i78 = phi i64 [ %.mux112, %powmod_ll.exit ], [ %pow.1.i76, %for.inc9.i75 ]
  %add5 = add i64 %retval.0.i78, %conv
  %.mux114 = select i1 %cmp.i, i64 1, i64 %add5
  br i1 %switch116, label %powmod_ll.exit99, label %for.body4.i83

for.body4.i83:                                    ; preds = %powmod_ll.exit79, %for.inc9.i95
  ...
  %rem.i88 = urem i64 %mul.i87, %t.addr.0108
...
if.then5.i92:                                     ; preds = %for.body4.i83
  ...
  %rem7.i94 = urem i64 %mul6.i93, %t.addr.0108
...
powmod_ll.exit99:                                 ; preds = %for.inc9.i95, %powmod_ll.exit79
  %retval.0.i98 = phi i64 [ %.mux114, %powmod_ll.exit79 ], [ %pow.1.i96, %for.inc9.i95 ]
  ...
  %mul = mul i64 %tmp.0, %q.0105
  %rem = urem i64 %mul, %t.addr.0108
  %cmp11 = icmp eq i32 %i.0107, %j.0109
  br i1 %cmp11, label %if.then13, label %for.inc

if.then13:                                        ; preds = %powmod_ll.exit99
  %add14 = add nsw i32 %jj.0110, 1
  %add15 = add nsw i32 %add14, %j.0109
  %call12.i = tail call i32 (...) @example7() #8
  %cmp13.i = icmp eq i32 %call12.i, 0
  br i1 %cmp13.i, label %gcd_ll.exit, label %if.end.i

if.end.i:                                         ; preds = %if.then13, %if.end5.i
  %y.addr.015.i = phi i64 [ %rem6.i, %if.end5.i ], [ %t.addr.0108, %if.then13 ]
  %x.addr.014.i = phi i64 [ %rem.i100, %if.end5.i ], [ %rem, %if.then13 ]
  %call1.i = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, i64 noundef %x.addr.014.i)
  %conv.i = sext i32 %call1.i to i64
  %rem.i100 = urem i64 %conv.i, %y.addr.015.i
  %cmp2.i = icmp eq i64 %rem.i100, 0
  br i1 %cmp2.i, label %gcd_ll.exit, label %if.end5.i

if.end5.i:                                        ; preds = %if.end.i
  %rem6.i = urem i64 %y.addr.015.i, %rem.i100
  %call.i = tail call i32 (...) @example7() #8
  %cmp.i101 = icmp eq i32 %call.i, 0
  br i1 %cmp.i101, label %gcd_ll.exit, label %if.end.i, !llvm.loop !5

gcd_ll.exit:                                      ; preds = %if.end.i, %if.end5.i, %if.then13
  %retval.0.i102 = phi i64 [ %rem, %if.then13 ], [ %rem.i100, %if.end5.i ], [ %y.addr.015.i, %if.end.i ]
  %cmp17.not = icmp eq i64 %retval.0.i102, 1
  br i1 %cmp17.not, label %for.inc, label %if.then19

if.then19:                                        ; preds = %gcd_ll.exit
  %incdec.ptr = getelementptr inbounds nuw i8, ptr %xp.0106, i64 8
  store i64 %retval.0.i102, ptr %xp.0106, align 8, !tbaa !9
  %div = udiv i64 %t.addr.0108, %retval.0.i102
  %cmp20 = icmp eq i64 %div, 1
  br i1 %cmp20, label %cleanup, label %for.inc

for.inc:                                          ; preds = %powmod_ll.exit99, %if.then19, %gcd_ll.exit
  %xp.1 = phi ptr [ %incdec.ptr, %if.then19 ], [ %xp.0106, %gcd_ll.exit ], [ %xp.0106, %powmod_ll.exit99 ]
  %t.addr.1 = phi i64 [ %div, %if.then19 ], [ %t.addr.0108, %gcd_ll.exit ], [ %t.addr.0108, %powmod_ll.exit99 ]
  %j.1 = phi i32 [ %add15, %if.then19 ], [ %add15, %gcd_ll.exit ], [ %j.0109, %powmod_ll.exit99 ]
  %jj.1 = phi i32 [ %add14, %if.then19 ], [ %add14, %gcd_ll.exit ], [ %jj.0110, %powmod_ll.exit99 ]
