; 195111370509214129188783758365622894757
; ModuleID = '/mnt/ramtmp/optims/DCE.cpp/target/195111370509214129188783758365622894757.c'
source_filename = "/mnt/ramtmp/optims/DCE.cpp/target/195111370509214129188783758365622894757.c"
...
for.cond:                                         ; preds = %if.end3, %entry
  %x.addr.0 = phi i64 [ %x, %entry ], [ %rem, %if.end3 ]
  %y.addr.0 = phi i64 [ %y, %entry ], [ %rem4, %if.end3 ]
  %cmp = icmp ugt i64 %y.addr.0, 5
...
for.cond.i:                                       ; preds = %if.end3.i, %if.then13
  %x.addr.0.i = phi i64 [ %rem, %if.then13 ], [ %rem.i101, %if.end3.i ]
  %y.addr.0.i = phi i64 [ %t.addr.0107, %if.then13 ], [ %rem4.i, %if.end3.i ]
  %cmp.i100 = icmp ugt i64 %y.addr.0.i, 5
  br i1 %cmp.i100, label %if.then.i, label %if.end.i
...
if.end.i:                                         ; preds = %if.then.i, %for.cond.i
  %rem.i101 = urem i64 %x.addr.0.i, %y.addr.0.i
  %cmp1.i = icmp eq i64 %rem.i101, 0
  br i1 %cmp1.i, label %gcd_ll.exit, label %if.end3.i

if.end3.i:                                        ; preds = %if.end.i
  %rem4.i = urem i64 %y.addr.0.i, %rem.i101
  br label %for.cond.i, !llvm.loop !5

gcd_ll.exit:                                      ; preds = %if.end.i
  %add15 = add nsw i32 %add14, %j.0108
  %cmp17.not = icmp eq i64 %y.addr.0.i, 1
...
if.then19:                                        ; preds = %gcd_ll.exit
  ...
  store i64 %y.addr.0.i, ptr %xp.0105, align 8, !tbaa !9
  %div = udiv i64 %t.addr.0107, %y.addr.0.i
